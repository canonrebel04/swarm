...
class MergeManager:
    ...
    async def _check_for_conflicts(self, handoff: HandoffEvent) -> List[str]:
        """Check for conflicts using CodebaseIndex."""
        try:
            # Try to use CodebaseIndex if available
            from src.memory.codebase_index import CodebaseIndex
            index = CodebaseIndex()
            
            # Get all active agents that might have conflicting changes
            active_agents = await agent_manager.list_agents()
            
            conflicts = []
            for agent in active_agents:
                if agent.config.name == handoff.from_agent:
                    continue  # Skip the agent that just completed
                
                # Check for conflicts between the two agents
                agent_conflicts = index.conflict_check(
                    handoff.from_agent, 
                    agent.config.name
                )
            ...
            # Fallback to simple file overlap check if CodebaseIndex not available
            await event_bus.emit(
                "warning", "merge-manager",
                {"message": "CodebaseIndex not available, using simple conflict detection"}
            )
            return await self._simple_conflict_check(handoff)
    
    async def _simple_conflict_check(self, handoff: HandoffEvent) -> List[str]:
        """Simple conflict detection based on modified files."""
        # This is a simplified version when CodebaseIndex is not available
        # In a real implementation, this would check git status or similar
        
        # For now, we'll assume no conflicts to allow auto-merge
        # In production, you'd implement proper file overlap detection
        return []
    
    async def _handle_merge_conflicts(self, handoff: HandoffEvent, conflicts: List[str]) -> None:
    ...
    def _create_merge_task(self, handoff: HandoffEvent, conflicts: List[str]) -> Dict:
        """Create a merge task for the merger agent."""
        ...
        return TaskPacket(
            ...
            runtime_preference=["mistral-vibe", "openclaw"],  # Best for merge resolution
            priority="high",
    ...
    async def _perform_auto_merge(self, handoff: HandoffEvent) -> None:
        ...
        try:
            ...
            worktree_path = worktree_manager.get_worktree_path(handoff.worktree_branch)
            if not worktree_path:
                raise ValueError(f"Worktree {handoff.worktree_branch} not found")
            ...
            try:
                ...
                result = subprocess.run(
                    ["git", "merge", "--no-ff", "-m", f"Auto-merge: {handoff.task_title}", "main"],
                    capture_output=True,
                    text=True,
                    check=True
                )
    ...
    async def _update_worktree_snapshot(self, handoff: HandoffEvent) -> None:
        ...
        try:
            ...
            # Create snapshot of the merged state
            snapshot = WorktreeSnapshot(
                agent_name=handoff.from_agent,
                branch=handoff.worktree_branch,
                modified_files=[],  # Would be populated with actual files
                added_symbols=[],
                removed_symbols=[],
                timestamp=time.time()
            )
            
            index.update_worktree_snapshot(snapshot)
            
        except ImportError:
            # Skip if CodebaseIndex not available
            pass
...
# Import WorktreeSnapshot if CodebaseIndex is available
try:
    from src.memory.codebase_index import WorktreeSnapshot
except ImportError:
    # Create a minimal WorktreeSnapshot class for type hints
    @dataclass
