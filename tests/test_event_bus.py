...
class TestEventBusPersistence:
    ...
    async def test_event_persistence(self, event_bus):
        ...
        cursor = await event_bus.db._conn.execute("SELECT * FROM events")
        rows = await cursor.fetchall()
        assert len(rows) == 1

        # Verify data was stored correctly
        stored_data = json.loads(rows[0][4])  # data column
        assert stored_data["key"] == test_data["key"]
